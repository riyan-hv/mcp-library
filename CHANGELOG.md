# MCP Library Changelog

Auto-updated every Monday by the GitHub Actions workflow.
Manual additions welcome — open a PR to this repo.

---

## 2026-02-25

### 2455 New MCP Servers Discovered

- [awesome-mcp-clients](https://github.com/punkpeye/awesome-mcp-clients/)
- [1mcp/agent](https://github.com/1mcp-app/agent)
- [Aganium/agenium](https://github.com/Aganium/agenium)
- [espadaw/Agent47](https://github.com/espadaw/Agent47)
- [AgentHotspot](https://github.com/AgentHotspot/agenthotspot-mcp)
- [ariekogan/ateam-mcp](https://github.com/ariekogan/ateam-mcp)
- [askbudi/roundtable](https://github.com/askbudi/roundtable)
- [blockrunai/blockrun-mcp](https://github.com/blockrunai/blockrun-mcp)
- [Data-Everything/mcp-server-templates](https://github.com/Data-Everything/mcp-server-templates)
- [duaraghav8/MCPJungle](https://github.com/duaraghav8/MCPJungle)
- [edgarriba/prolink](https://github.com/edgarriba/prolink)
- [glenngillen/mcpmcp-server](https://github.com/glenngillen/mcpmcp-server)
- [hamflx/imagen3-mcp](https://github.com/hamflx/imagen3-mcp)
- [hashgraph-online/hashnet-mcp-js](https://github.com/hashgraph-online/hashnet-mcp-js)
- [isaac-levine/forage](https://github.com/isaac-levine/forage)
- [jaspertvdm/mcp-server-gemini-bridge](https://github.com/jaspertvdm/mcp-server-gemini-bridge)
- [jaspertvdm/mcp-server-ollama-bridge](https://github.com/jaspertvdm/mcp-server-ollama-bridge)
- [jaspertvdm/mcp-server-openai-bridge](https://github.com/jaspertvdm/mcp-server-openai-bridge)
- [julien040/anyquery](https://github.com/julien040/anyquery)
- [juspay/neurolink](https://github.com/juspay/neurolink)
- [K-Dense-AI/claude-skills-mcp](https://github.com/K-Dense-AI/claude-skills-mcp)
- [khalidsaidi/ragmap](https://github.com/khalidsaidi/ragmap)
- [merterbak/Grok-MCP](https://github.com/merterbak/Grok-MCP)
- [metatool-ai/metatool-app](https://github.com/metatool-ai/metatool-app)
- [mindsdb/mindsdb](https://github.com/mindsdb/mindsdb)
- [oxgeneral/agentnet](https://github.com/oxgeneral/agentnet)
- [particlefuture/MCPDiscovery](https://github.com/particlefuture/1mcpserver)
- [portel-dev/ncp](https://github.com/portel-dev/ncp)
- [profullstack/mcp-server](https://github.com/profullstack/mcp-server)
- [sitbon/magg](https://github.com/sitbon/magg)

_(and 2425 more — see known_servers.json)_
